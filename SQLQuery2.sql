...
delete from UserDetails where UserID = 34;

update UserDetails SET UserID = -1 where UserID is null;

alter table UserDetails add constraint PK_UserDetails primary key (UserID);

alter table UserDetails alter column UserID int not null;

create table DealerDetails (DealerID varchar(40) , DealerName varchar(40), PhoneNumber varchar(50), Email varchar(40)

constraint PK_Dealer primary key(DealerID));

select* from DealerDetails;

alter table DealerDetails add  Password_ varchar(40);select DD.DealerName, ER.BaseCurrency, ER.ToCurrency, TR.BasePrice, ER.ExchPer,  DD.PhoneNumber, DD.Email  from ExchangeRates ER  
